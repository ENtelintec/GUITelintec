...
def update_aptitud_renovacion(
    aptitud: list, renovaciones: list, apt_actual: int, exm_id: int, exam_id=None
):
    sql = (
        (
...
def update_aptitud(aptitud: list, apt_actual: int, emp_id: int = None, exam_id=None):
    ...
    val = (json.dumps(aptitud), apt_actual, emp_id) if exam_id is None else (json.dumps(aptitud), apt_actual, exam_id)
    flag, e, out = execute_sql(sql, val, 3)
    return flag, e, out
...
def update_renovacion(
    ...
    val = (json.dumps(renovaciones), emp_id) if exam_id is None else (json.dumps(renovaciones), exam_id)
    flag, e, out = execute_sql(sql, val, 3)
    return flag, e, out
...
def update_status_EM(status, emp_id):
    sql = (
        "UPDATE sql_telintec.examenes_med " "SET status = %s " "WHERE empleado_id = %s"
    )
    val = (status, emp_id)
    flag, e, out = execute_sql(sql, val, 4)
    return flag, e, out
