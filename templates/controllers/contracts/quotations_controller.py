...
def create_quotation(metadata: dict, products: dict):
    ...
    sql = ("INSERT INTO sql_telintec_mod_admin.quotations (metadata, products, creation) "
           "VALUES (%s, %s, %s)")
...
def update_quotation(id_quotation, metadata: dict, products: dict, timestamps=None):
    ...
    if timestamps is None:
        timestamps = {"complete": {"timestamp": "", "comment": ""}, "update": [timestamp]}
    else:
        timestamps["update"].append(timestamp)
    sql = ("UPDATE sql_telintec_mod_admin.quotations "
           "SET metadata = %s, products = %s, timestamps = %s  "
           "WHERE id = %s")
    val = (json.dumps(metadata), json.dumps(products), json.dumps(timestamps), id_quotation)
    flag, error, out = execute_sql(sql, val, 3)
    return flag, error, out


def delete_quotation(id_quotation):
    sql = ("DELETE FROM sql_telintec_mod_admin.quotations "
           "WHERE id = %s")
    val = (id_quotation,)
    flag, error, out = execute_sql(sql, val, 3)
    return flag, error, out
...
def get_quotation(id_quotation=None):
    if id_quotation is None:
        sql = ("SELECT id, metadata, products, creation, timestamps "
               "FROM sql_telintec_mod_admin.quotations")
        flag, error, result = execute_sql(sql, None, 5)
        if not flag:
            return False, error, None
        return True, None, result
    sql = ("SELECT id, metadata, products, creation "
           "FROM sql_telintec_mod_admin.quotations "
           "WHERE id = %s")
    val = (id_quotation,)
