...
def create_contract(id_quotation, metadata: dict):
    ...
    sql = ("INSERT INTO sql_telintec_mod_admin.contracts (metadata, creation, quotation_id) "
           "VALUES (%s, %s, %s)")
...
def update_contract(id_contract, metadata: dict, timestamps=None):
    ...
    sql = ("UPDATE sql_telintec_mod_admin.contracts "
...
def delete_contract(id_contract):
    sql = ("DELETE FROM sql_telintec_mod_admin.contracts "
           "WHERE id = %s")
    val = (id_contract,)
    flag, error, out = execute_sql(sql, val, 3)
    return flag, error, out
...
def get_contract(id_contract=None):
    if id_contract is None:
        sql = ("SELECT id, metadata, creation, quotation_id, timestamps "
