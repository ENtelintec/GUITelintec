...
def get_isAlive(chat_id: int, sender_id):
    sql = (
        "SELECT is_alive FROM sql_telintec.chats WHERE chat_id = %s AND sender_id = %s"
    )
...
def get_only_context(chat_id: str):
    sql = "SELECT context FROM sql_telintec.chats WHERE chat_id = %s"
    val = (chat_id,)
    flag, error, result = execute_sql(sql, val, 1)
    return result
...
def set_finish_chat(chat_id: str):
    sql = "UPDATE sql_telintec.chats SET is_alive = 0, is_review = 1 WHERE chat_id = %s"
    val = (chat_id,)
    flag, error, result = execute_sql(sql, val, 3)
    return result
...
def get_chats_w_limit(limit=(0, 100)):
    sql = "SELECT chat_id, platform, context FROM sql_telintec.chats ORDER BY chat_id DESC LIMIT %s, %s"
    val = (limit[0], limit[1])
