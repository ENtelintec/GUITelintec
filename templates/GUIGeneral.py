...
class GUIAsistente(ttk.Window):
    def __init__(self, master=None, *args, **kwargs):
        ...
        self.theme_selector = ttk.Combobox(
            frame_theme, values=theme_names, state="readonly", width=15
        )
    ...
    def get_username_data(self, username_data=None, permissions=None):
        self.username_data = (
            get_username_data(self.username) if username_data is None else username_data
        )
        self.username_data["permissions"] = (
            self.permissions if permissions is None else permissions
        )

    def update_side_menu(self, windows_names):
        self._destroy_side_menu_widgets() if self.buttons_side_menu is not None else None
        self.buttons_side_menu, self.names_side_menu = self._create_side_menu_widgets(
            self.side_menu_frame, windows_names
        )
    ...
    def update_side_menu_windows(self, data_dic):
        ...
        department = (
            self.username_data["department_name"]
            if self.username_data is not None
            else "default"
        )
        permissions_allowed_av = json.load(open(ventanasApp_path, encoding="utf-8"))[
    ...
    def create_AV_window(self, department):
        ...
        self.virtual_assistant_window = AssistantGUI(
            self.VA_frame, department=department, width=150
        )
        ...
        match name:
            ...
            case _:
                ...
                if self._active_window != name:
                    self.windows_frames[
                        self._active_window
                    ].grid_forget() if self._active_window is not None else None
                    self._active_window = name
                    self.windows_frames[name].grid(
    ...
    def _create_side_menu_widgets(self, master, windows_names=None):
        ...
        if len(windows_names) >= 12:
            side_menu = SideMenuFrameScrollable(
                master, windows_names, flag, self._select_frame_by_name, width=200
            )
            side_menu.grid(row=0, column=0, sticky="ns", pady=10, padx=(0, 5))
        else:
            side_menu = SideMenuFrame(
                master, windows_names, flag, self._select_frame_by_name, width=200
            )
...
class LogoFrame(tk.Frame):
    def __init__(self, master):
        ...
        canva.create_image(
            width_img / ratio, height_img / ratio, anchor="s", image=img_logo
        )
