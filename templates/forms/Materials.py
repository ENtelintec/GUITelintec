...
from templates.forms.PDFGenerator import a4_y, a4_x, create_header_materials, create_footer_sign, create_header, \
...
def print_headers_table_inventory(pdf, font_size=10, y_init=635, type_form="ReturnMaterials"):
    """
    :param type_form: 
    :param y_init: 
    :param pdf:
...
def ReturnMaterialsForm(dict_data: dict, type_form="ReturnMaterials"):
    ...
    create_header_materials(pdf, title="DEVOLUCION DE MATERIALES", page_x=a4_x, date_int=dict_data["date_emision"],
                            type_form=3, info_dict=dict_data["info"])
    pages = 0
    ...
    if calculate_last_y(item, limit_y, font_size, y_init, type_form=type_form, width=int(a4_x/(font_size*0.7))):
        pages += 1
        print_footer_page_count(pdf, pages)
        pdf.showPage()
        y_init = 780
    value = textwrap.wrap(str(dict_data["observations"]), width=int(a4_x/(font_size*0.7)))
    for letter in value:
...
def MaterialsRequest(dict_data: dict, type_form="MaterialsRequest"):
    ...
    :return:
    """
    file_name = "files/materials_request.pdf" if dict_data["filename_out"] is None else dict_data["filename_out"]
    pdf = canvas.Canvas(file_name, pagesize=(a4_x, a4_y))
    limit_y = 10
