...
def tranform_data_vacations_db_display(data):
    ...
    for entry in data:
        ...
        prima, prima_pago, comentarios, status = fill_empty_with_None(
            6, (prima, prima_pago, comentarios, status)
        )
...
class VacationsFrame(ScrolledFrame):
    def __init__(self, master, setting: dict = None, *args, **kwargs):
        ...
        self.data, columns = (
            self.load_data_vacations(1)
            if "vacaciones" not in kwargs["data"]
            else (
                kwargs["data"]["vacaciones"]["data"],
                kwargs["data"]["vacaciones"]["columns"],
            )
        )
        ...
        (
            self.wentry_emp_id,
            self.wentry_year,
            self.wentry_pendientes,
            self.wentry_prima,
            self.wentry_prima_pago,
            self.wentry_comentarios,
        ...
        ttk.Button(
            ...
            command=self._update_registry_to_emp,
    ...
    def _update_emps(self):
        ...
        if flag:
            ...
            if len(ids_not_vac) == 0:
                ...
                return
            complete_name_of_not_vac = [
                item[1].upper() + " " + item[2].upper()
                for item in data
                if item[0] in ids_not_vac
            ]
    ...
    def _test_id(self):
        ...
        if emp_id == "":
            Messagebox.show_error(
                title="Error", message="El id del empleado esta vacio."
            )
            return
        ...
        if name is not None:
            Messagebox.show_info(
                title="Exito", message="El id del empleado es correcto."
            )
            self.name_emp.set(f"ID: {emp_id}\nNombre: {name}")
        else:
            Messagebox.show_error(
                title="Error", message="El id del empleado es incorrecto."
            )
    ...
    def create_input_widgets(self):
        ...
        create_label(
            frame_inputs,
            textvariable=self.year_var_info,
            row=3,
            column=0,
            padx=0,
            pady=0,
        ...
        entry_year = ttk.Spinbox(
            frame_inputs, from_=1, to=80, state="readonly", command=self._test_year
        )
        ...
        entry_prima = create_Combobox(
            frame_inputs, ["No", "Si"], row=1, column=3, sticky="w", padx=7, pady=5
        )
        ...
        entry_comentarios.grid(
            row=1, column=4, sticky="nsew", padx=5, pady=5, rowspan=2
        )
        return (
            entry_emp_id,
            entry_year,
    ...
    def on_double_click(self, event):
        ...
        if len(seniority_dict) > 0:
            self.seniority_dict = seniority_dict
            self.wentry_comentarios.insert(
                0.0, seniority_dict["0"]["comentarios"]
            ) if seniority_dict["0"][
                "comentarios"
            ] is not None else self.wentry_comentarios.insert(0.0, "")
            self.wentry_prima.set("Si") if seniority_dict["0"]["prima"][
                "status"
            ].lower() == "si" else self.wentry_prima.set("No")
            txt_pendientes = seniority_dict["0"]["status"]
            self.wentry_pendientes.set(
                int(txt_pendientes.split(" ")[0])
            ) if txt_pendientes.lower() != "tomadas" else self.wentry_pendientes.set(0)
            # noinspection PyArgumentList
            self.wentry_pendientes.configure(
                bootstyle="success"
            ) if txt_pendientes.lower() == "tomadas" else self.wentry_pendientes.configure(
                bootstyle="danger"
            )
        ...
        if self.wentry_year.get() == "" or self.wentry_emp_id.get() == "":
            ...
            if self.wentry_year.get() == "0":
                ...
                try:
                    ...
                    Messagebox.show_error(
                        title="Error", message="El ID del empleado debe ser un numero"
                    )
                    ...
                    if len(seniority_dict) > 0:
                        for item in seniority_dict.keys():
                            if int(item) == year - 1 and year > 0:
                                self.clean_widgets_year()
                                self.wentry_comentarios.insert(
                                    0.0, seniority_dict[item]["comentarios"]
                                ) if seniority_dict[item][
                                    "comentarios"
                                ] is not None else self.wentry_comentarios.insert(
                                    0.0, ""
                                )
                                self.wentry_prima.set("Si") if seniority_dict[item][
                                    "prima"
                                ...
                                self.wentry_pendientes.set(
                                    int(txt_pendientes.split(" ")[0])
                                ) if txt_pendientes.lower() != "tomadas" else self.wentry_pendientes.set(
                                    0
                                )
                                self.pagoprima_string.set(
                                    f"{seniority_dict[item]['prima']['fecha_pago']}"
                                )
                        ...
                        self.year_var_info.set(
                            "No hay registro de vacaciones del empleado"
                        )
                        self.clean_widgets_year()
                else:
