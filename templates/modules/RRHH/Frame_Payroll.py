...
from templates.Functions_GUI_Utils import create_label, create_Combobox, create_button, create_entry
from templates.Functions_Sharepoint import create_mail_draft_with_attachment, download_files_site
from templates.controllers.employees.employees_controller import get_emp_mail
from templates.controllers.payroll.payroll_controller import update_payroll, update_payroll_employees, \
...
class PayrollFilesGUI(ttk.Frame):
    def __init__(self, master=None, **kwargs):
        ...
        create_label(self, text='Documentos de Nomina', row=0, column=0, font=("Arial", 18))
    ...
    def create_inputs(self, master):
        emps_names = [emp[1].upper() + ' ' + emp[2].upper() for emp in self.payrolls_info["payrolls"]]
        emps_names.sort()
    ...
    def create_buttons(self, master):
        create_button(master, text='Actualizar archivos', row=0, column=0, command=self.refresh_data)
        create_button(master, text='Crear Correo', row=0, column=1, command=self.create_email)
        create_button(master, text='Descargar archivos', row=0, column=2, command=self.create_files)
        create_button(master, text='Actualizar Empleados', row=0, column=3, command=self.update_employees)
        create_button(master, text='Actualizar Tabla', row=0, column=4, command=self.update_data_table)
    ...
    def create_table_display(self):
        ...
        table = Tableview(master, bootstyle="primary",
                          coldata=coldata, rowdata=data,
                          paginated=False,
                          searchable=True,
                          autofit=False)
    ...
    def create_draft(self, destiny_mail, subject, body, source_email):
        ...
        download_path_xml, code = download_files_site(url_shrpt + folder_rrhh, self.data_file_emp["xml"])
        download_path_pdf, code = download_files_site(url_shrpt + folder_rrhh, self.data_file_emp["pdf"])
        temp_files = [download_path_xml, download_path_pdf]
        response, code = create_mail_draft_with_attachment(
            self.id_emp_edit, source_email, subject, body,
            temp_files, to_recipients=destiny_mail)
        if code == 200:
        ...
        self.data_emp_dict[self.entries[1].get()][str(month)][self.selected_file]["is_send"] = 1
        flag, error, result = update_payroll(self.data_emp_dict, self.id_emp_edit)
        if flag:
            msg = f"Base de datos actualizada correctamente para emp: {self.id_emp_edit}"
            Messagebox.show_info(title="Correo Creado", message=msg)
        else:
    ...
    def update_selected_file(self, event):
        ...
        month = self.entries[2].get()
        month = transform_string_month_to_int([month])[0]
        self.data_file_emp = self.data_emp_dict[self.entries[1].get()][str(month)][self.selected_file]
        # read xml data
        settings = json.load(open("files/settings.json", "r"))
        url_shrpt = settings["gui"]["RRHH"]["url_shrpt"]
        folder_rrhh = settings["gui"]["RRHH"]["folder_rrhh"]
        ...
        if "is_send" in self.data_file_emp.keys():
            self._svar_txt_sent.set("Enviado") if self.data_file_emp["is_send"] == 1 else self._svar_txt_sent.set("No Enviado")
        else:
...
class MailCreation(ttk.Toplevel):
    def __init__(self, master=None, email_source=None, emp_destiny_id=None, **kwargs):
        ...
        create_label(self, text='Ingrese su correo institucional[@telintec.com.mx]: ', row=4, column=0)
...
class EditPatternsUpdateDataPayroll(ttk.Toplevel):
    def __init__(self, master=None, **kwargs):
        ...
        create_button(self, text='Actualizar', row=3, column=1, command=self.update_data_action)
