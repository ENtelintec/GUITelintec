...
from templates.misc.Functions_Files import (
    ...
    extract_data_file_contracts,
    generate_table_from_dict_contracts,
)
...
class FichajesManual(ttk.Frame):
    def __init__(self, master=None, *args, **kwargs):
        ...
        (
            self.svar_wd_f,
            self.svar_late_f,
            self.svar_nday_extra_f,
            self.svar_wd_intime_f,
            self.svar_late_hours_f,
            self.svar_tot_extra_hours_f,
            self.svar_late_h_by_day_f,
            self.svar_late_hday_comment_f,
            self.svar_extra_h_by_day_f,
            self.svar_extra_hday_comment_f,
            self.svar_out_door_f,
            self.svar_in_door_f,
            self.svar_missed_days,
            self.svar_late_op,
            ...
            self.svar_wd_t,
            self.svar_late_t,
            self.svar_nday_extra_t,
            self.svar_wd_intime_t,
            self.svar_late_hours_t,
            self.svar_tot_extra_hours_t,
            self.svar_late_h_by_day_t,
            self.svar_late_hday_comment_t,
            self.svar_extra_h_by_day_t,
            self.svar_extra_hday_comment_t,
            self.svar_out_door_t,
            self.svar_in_door_t,
        ...
        (
            self.clocks_f,
            self.window_time_in_f,
            self.window_time_out_f,
            self.label_time_in_f,
            self.label_time_out_f,
            self.name_emp_selector_f,
        ) = self.create_inputs_f(frame_inputs1)
        # -------------------resume info
        ...
        self.days_late_selector_f, self.days_extra_selector_f = (
            self.create_info_display_file1(frame_info1, 0)
        )
        ...
        self.name_oct_selector, self.date_selector_bit = self.create_inputs_b(
            frame_inputs2
        )
        ...
        (
            self.days_missing_selector2,
            self.days_late_selector2,
            self.days_extra_selector2,
            self.days_primas_selector,
        ) = self.create_info_display_file2(frame_info2)
        ...
        (
            self.clocks_t,
            self.window_time_in_t,
            self.window_time_out_t,
            self.label_time_in_t,
            self.label_time_out_t,
            self.name_emp_selector_t,
        ) = self.create_inputs_f(frame_inputs3, True)
        # -------------------resume info
        ...
        self.days_late_selector_t, self.days_extra_selector_t = (
            self.create_info_display_file1(frame_info3, 1)
        )
    ...
    def create_info_display_file1(self, master, type_i=0):
        ...
        create_label(
            master,
            text="Información resumida archivo principal",
            row=5,
            column=0,
            columnspan=6,
        )
        ...
        if type_i == 0:
            ...
            days_extra_selector.bind(
                "<<ComboboxSelected>>", self.select_day_extra_fun_f
            )
        else:
    ...
    def create_info_display_file2(self, master):
        create_label(
            master,
            text="Información resumida archivos secundario",
            row=3,
            column=0,
            columnspan=6,
        )
        ...
        create_label(
            master,
            textvariable=self.svar_com_missed_days,
            row=4,
            column=2,
            font=("Helvetica", 11),
            columnspan=4,
        ...
        create_label(
            master,
            textvariable=self.svar_com_late_op,
            row=5,
            column=2,
            font=("Helvetica", 11),
            columnspan=4,
        ...
        create_label(
            master,
            textvariable=self.svar_com_extra_op,
            row=6,
            column=2,
            font=("Helvetica", 11),
            columnspan=4,
        ...
        create_label(
            master,
            textvariable=self.svar_com_primas_op,
            row=7,
            column=2,
            font=("Helvetica", 11),
            columnspan=4,
        ...
        return (
            days_missing_selector2,
            days_late_selector2,
            days_extra_selector2,
            days_primas_selector,
        )
    ...
    def select_day_late_fun_f(self, event):
        if event.widget.get() != "no data":
            ...
            getawey = df_name[
                (df_name["Fecha/hora_out"] > limit_hour_down)
    ...
    def select_day_extra_fun_f(self, event):
        if event.widget.get() != "no data":
            ...
            hours, minutes = divmod(aux / 60, 60)
            self.svar_extra_h_by_day_f.set(
                f"Horas extras: \n{int(hours)} con {int(minutes)} minutos."
            )
            self.svar_out_door_f.set(
            ...
            entrance = df_name[
                (df_name["Fecha/hora_in"] > limit_hour_down)
    ...
    def select_day_late(self, event):
        if event.widget.get() != "no data":
            ...
            getawey = df_name[
                (df_name["Fecha/hora"] > limit_hour_down)
    ...
    def select_day_extra(self, event):
        if event.widget.get() != "no data":
            ...
            hours, minutes = divmod(aux / 60, 60)
            self.svar_extra_h_by_day_t.set(
                f"Horas extras: \n{int(hours)} con {int(minutes)} minutos."
            )
            self.svar_out_door_t.set(
        ...
        if event.widget.get() != "no name selected":
            ...
            data_chart = {
                "data": {
                    "Tarde": count,
                    "Extras": count2,
                    "A tiempo": worked_intime,
                    "Trabajado": worked_days,
                },
    ...
    def on_name_selected_bitacora(self, event):
        ...
        if event.widget.get() != "no name selected":
            ...
            data_chart = {
                "data": {
                    "Tarde": days_lates_bit[0],
                    "Extras": days_extra_bit[0],
                    "Faltas": days_absence_bit[0],
                    "Primas": days_primes_bit[0],
                },
                "title": f"Datos de {event.widget.get()}",
                "ylabel": "Dias",
            }
    ...
    def change_time_grace(self, event):
        if self.file_selected_1 or self.file_selected_3:
            self.label_time_in_f.configure(
                text=f"Gracia: {int(self.window_time_in_f.get())} mins"
            )
            self.label_time_out_f.configure(
                text=f"Gracia: {int(self.window_time_out_f.get())} mins"
            )
        if self.file_selected_3:
            self.label_time_in_t.configure(
                text=f"Gracia: {int(self.window_time_in_t.get())} mins"
            )
            self.label_time_out_t.configure(
                text=f"Gracia: {int(self.window_time_out_t.get())} mins"
            )
    ...
    def create_inputs_f(self, master, typet=False):
        ...
        if typet:
            file_fichaje_selector = create_Combobox(
                master, values=self.files_ternium, row=0, column=1
            )
            name_emp_selector.bind("<<ComboboxSelected>>", self.select_name_emp_t)
        else:
            file_fichaje_selector = create_Combobox(
                master, values=self.files_fichaje, row=0, column=1
            )
        ...
        clock1, dict_c = create_spinboxes_time(
            master, self, 3, 0, mins_defaul=0, hours_default=8, title="entrada"
        )
        ...
        clock2, dict_c = create_spinboxes_time(
            master, self, 3, 2, mins_defaul=0, hours_default=18, title="salida"
        )
