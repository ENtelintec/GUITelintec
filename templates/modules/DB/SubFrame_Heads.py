...
class HeadsFrame(ttk.Frame):
    def __init__(self, master, setting: dict = None, **kwargs):
        ...
        self.label = ttk.Label(
            self, text="Tabla de Jefes de Departamento", font=("Helvetica", 32, "bold")
        )
        ...
        # ---------------widgets for insert-----------------------
        self.entries = create_widget_input_DB(self.insert_frame, "heads")
        ...
        self.label_help = ttk.Label(
            self.visual_frame,
            text="Mirar las sisguientes tablas para referencias de los IDs.",
        )
    ...
    def _insert_head(self):
        ...
        msg = (
            f"Are you sure you want to insert the following head:\n"
            f"Name: {name}\nEmployee: {employee}\nDepartment: {department}"
        )
        answer = Messagebox.show_question(title="Confirmacion", message=msg)
        if answer == "No":
            return
        flag, e, out = insert_head_DB(name, department, employee)
        if flag:
    ...
    def _update_head(self):
        ...
        name, department, employee = self.get_entries_values()
        msg = (
            f"Are you sure you want to update the following head:\n"
            f"Name: {name}\nEmployee: {employee}\nDepartment: {department}"
        )
        answer = Messagebox.show_question(title="Confirmacion", message=msg)
        if answer == "No":
    ...
    def _delete_head(self):
        if self._id_heads_update is None:
            ...
            return
        msg = (
            f"Are you sure you want to delete the following head:\n"
    ...
    def get_entries_values(self):
        return (self.entries[0].get(), self.entries[1].get(), self.entries[2].get())
    ...
    def _update_table_show(self, data):
        ...
        self.table_insert, self.data = create_visualizer_treeview(
            self.visual_frame, "heads", row=1, column=0, style="success", data=data
        )
        self.table_insert.view.bind("<Double-1>", self._get_data_head)
