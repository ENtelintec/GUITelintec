...
class EmployeesFrame(ScrolledFrame):
    def __init__(self, master, setting: dict = None, **kwargs):
        ...
        self.data = (
            kwargs["data"]["data_emps_gen"]
            if "data_emps_gen" in kwargs["data"]
            else None
        )
        # -----------------------label-----------------------
        self.label = ttk.Label(
            self, text="Employees Table", font=("Helvetica", 30, "bold")
        )
        ...
        self.entries = create_widget_input_DB(self.insert_frame, "employees")
        # -----------------------insert button-----------------------
        ...
        self.label_table1 = ttk.Label(self.visual_frame, text="Table Employees")
        ...
        for index, entry in enumerate(self.entries):
            if isinstance(entry, ttk.Combobox):
                ...
                if index == 13:
                    ...
                    value_update = (
                        departure["reason"] if departure["reason"] != "None" else ""
                    )
                elif index in [16, 17]:
                    emails = (
                        data[16].split(",")
                    ...
                    value_update = (
                        values[index - 18] if values[index - 18] is not None else ""
                    )
                elif index >= 14:
                ...
                if index == 7:
                    entry_date = (
                        datetime.strptime(data[index + 1], format_date)
    ...
    def _insert_employee(self):
        ...
        if values[0] == "" or values[1] == "":
            Messagebox.show_error(
                title="Error", message="Por favor rellenar la información."
            )
            return
        if status == "inactivo":
            msg = (
                f"Are you sure you want to insert the following employee:\n"
        ...
        if flag:
            ...
            Messagebox.show_info(
                title="Informacion", message=f"New employee created:\n{out}"
            )
        else:
            Messagebox.show_error(
                title="Error", message=f"Error creating employee:\n{e}"
            )

    def _update_employee(self):
        if self._id_emp_update is None:
            Messagebox.show_error(
                title="Error", message="Please select a employee to update."
            )
            return
        values = self.get_entries_values()
        status = values[11]
        if status == "inactivo":
            msg = (
                f"Are you sure you want to update the following employee to inactive:\n"
                f"Name: {values[0].upper()}\nLastname: {values[1].upper()}"
            )
        else:
            departure = {"date": "None", "reason": ""}
            msg = (
                f"Are you sure you want to update the following employee:\n"
                f"Name: {values[0].upper()}\nLastname: {values[1].upper()}\n"
            )
        answer = Messagebox.show_question(title="Confirmacion", message=msg)
        if answer == "No":
        ...
        if flag:
            ...
            Messagebox.show_error(
                title="Error", message=f"Error updating employee:\n{e}"
            )

    def _delete_employee(self):
        if self._id_emp_update is None:
            Messagebox.show_error(title="Error", message="No ha escogido un empleado")
            return
        ...
        if flag:
            ...
            self._update_table_show(self.data)
            Messagebox.show_info(
                title="Informacion", message=f"Employee deleted:\n{out}"
            )
            self.clean_widgets_emp()
        else:
            Messagebox.show_error(
                title="Error", message=f"Error deleting employee:\n{e}"
            )
