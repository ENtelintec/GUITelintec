...
from templates.controllers.tickets.tickets_controller import (
    insert_ticket_db,
    update_ticket_db,
    delete_ticket_db,
)
...
class TicketsFrame(ttk.Frame):
    def __init__(self, master, setting: dict = None, *args, **kwargs):
        ...
        self.label = ttk.Label(
            self, text="Tickets table", font=("Helvetica", 30, "bold")
        )
        ...
        self.entries = create_widget_input_DB(self.insert_frame, "tickets")
        # -----------------------buttons-----------------------
        ...
        self.tickets_insert, self.data = create_visualizer_treeview(
            self.visual_frame, "tickets", row=1, column=0, style="success"
        )
    ...
    def _insert_tickets(self):
        ...
        msg = (
            f"Are you sure you want to insert the following ticket:\n"
            f"Id ticket: {id_t}\n"
            f"Content: {content}\n"
        ...
        if flag:
            ...
            Messagebox.show_error(
                title="Error", message=f"Error inserting ticket:\n{e}"
            )
            return
    ...
    def _update_tickets(self):
        ...
        is_answered = int(is_answered)
        msg = (
            f"Are you sure you want to update the following ticket:\n"
            f"Id ticket: {id_t}\n"
            f"Content: {content}\n"
            f"Is retrieved: {is_retrieved}\n"
            f"Is answered: {is_answered}\n"
    ...
    def _delete_tickets(self):
        ...
        id_t = self._id_ticket_update
        msg = (
            f"Are you sure you want to delete the following ticket:\n"
            f"Id ticket: {id_t}"
        )
        answer = Messagebox.show_question(title="Confirmacion", message=msg)
        if answer == "No":
