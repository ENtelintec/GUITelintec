...
from templates.controllers.customer.customers_controller import insert_customer, update_customer_DB, delete_customer_DB
...
class CustomersFrame(ttk.Frame):
    ...
    def _insert_customer(self):
        ...
        if answer == "No":
            return
        flag, e, out = insert_customer(name, lastname, phone, email, ciudad)
        if flag:
    ...
    def _delete_customer(self):
        ...
        if answer == "No":
            return
        flag, e, out = delete_customer_DB(self._id_cus_update)
        if flag:
            for index, item in enumerate(self.data):
                if item[0] == self._id_cus_update:
                    self.data.pop(index)
