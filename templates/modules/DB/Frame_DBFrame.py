...
from static.constants import windows_names_db_frame
...
class DBFrame(ttk.Frame):
    ...
    def _create_side_menu_windows(self):
        ...
        for i, window in enumerate(self.names_side_menu):
            match window:
                case "Encargados":
                    windows[window] = HeadsFrame(
                        self, setting=self.settings, **self.kwargs
                    )
                    print("heads frame created")
                case "Clientes":
                    windows[window] = CustomersFrame(
                        self, setting=self.settings, **self.kwargs
                    )
                    print("customers frame created")
                case "Empleados":
                    windows[window] = EmployeesFrame(
                        self, setting=self.settings, **self.kwargs
                    )
                    print("employees frame created")
                case "Departamentos":
                    windows[window] = DepartmentsFrame(
                        self, setting=self.settings, **self.kwargs
                    )
                    print("departments frame created")
                case "Proveedores":
                    windows[window] = SuppliersFrame(
                        self, setting=self.settings, **self.kwargs
                    )
                    print("suppliers frame created")
                case "Productos":
                    windows[window] = ProductsFrame(
                        self, setting=self.settings, **self.kwargs
                    )
                    print("products frame created")
                case "Ordenes":
                    windows[window] = OrdersFrame(
                        self, setting=self.settings, **self.kwargs
                    )
                    print("orders frame created")
                case "O. Virtuales":
                    windows[window] = VOrdersFrame(
                        self, setting=self.settings, **self.kwargs
                    )
                    print("virtual orders frame created")
                case "Chats":
                    windows[window] = ChatsFrame(
                        self, setting=self.settings, **self.kwargs
                    )
                    print("chats frame created")
                case "Tickets":
                    windows[window] = TicketsFrame(
                        self, setting=self.settings, **self.kwargs
                    )
