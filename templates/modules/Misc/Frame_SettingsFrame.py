...
from templates.modules.Misc.SubFrame_Settings import (
    SettingsRRHH,
    SettingsChatbot,
    SettingsGeneral,
...
class SettingsFrameGUI(ttk.Frame):
    def __init__(
        self, master, style_gui, department=None, setting: dict = None, *args, **kwargs
    ):
        ...
        # --------------------------main frame----------------------------------
        flag, self.settings = (
            open_file_settings(filepath_settings)
            if setting is None
            else (True, setting)
        )
        create_label(
        ...
        self.settings_frame = avaliable_settings_frame["general"](
            self, self.settings, self.department, self.style_gui
        )
        ...
        create_button(
            frame_btns,
            text="Guardar",
            command=self.save_settings_master,
            width=10,
            row=0,
            column=0,
            sticky="n",
        ...
        create_button(
            frame_btns,
            text="Cargar",
            command=self.load_settings_master,
            width=10,
            row=0,
            column=1,
            sticky="n",
        ...
        for permission in kwargs["permissions"].values():
            ...
            if key_val.lower() in avaliable_settings_frame.keys():
                secondary_frame = avaliable_settings_frame[key_val.lower()](
                    self, self.settings, self.department, self.style_gui
                )
                secondary_frame.grid(
                    row=index, column=0, columnspan=4, padx=10, pady=10, sticky="nsew"
                )
