...
class TestingFrame(ttk.Frame):
    def __init__(self, master=None, **kw):
        ...
        # ----------------------------------------------create title--------------------------------------------------
        create_label(
            self, 0, 0, text="Testing", font=("Helvetica", 24, "bold"), sticky="n"
        )
        # ----------------------------------------------------create inputs----------------------------------------
        ...
        self.selector_event = create_Combobox(
            frame_inputs,
            row=1,
            column=0,
            values=["Faltas", "Atrasos", "Extras", "Primas"],
            state="readonly",
            sticky="n",
        )
        ...
        self.selector_model_class = create_Combobox(
            frame_inputs,
            row=5,
            column=0,
            values=models[0],
        ...
        self.selector_model_linear = create_Combobox(
            frame_inputs,
            row=6,
            column=0,
            values=models[1],
        ...
        self.selector_scalar = create_Combobox(
            frame_inputs,
            row=5,
            column=1,
            values=scalars,
        ...
        self.selector_data = create_Combobox(
            frame_inputs,
            row=5,
            column=2,
            values=data_test,
        ...
        create_button(
            frame_buttons, 0, 0, text="Predecir", command=self.test_model, sticky="n"
        )
        create_button(
            frame_buttons, 0, 1, text="Actualizar", command=self.read_files, sticky="n"
        )
    ...
    def test_model(self):
        ...
        y_pred, x_news, error, x_emps = test_model_fichajes(
            filepath_model_class,
            filepath_scalar,
            filepath_out_vectors,
            day_of_week,
            month,
            types_model=(1, 2),
            filepath_model_linear=filepath_model_linear,
        )
        ...
        for index, item in enumerate(y_pred[0]):
            if item > threshold and y_pred[1][index] > 0.25:
                counter += 1
                msg += (
                    f"El empleado con ID {int(x_emps[index, 0])} es posible que tenga un evento el dia {days_of_week_dict_reverse[day_of_week]} "
    ...
    def update_threshold(self, event):
        ...
        models[0] = (
            models[0] if len(models[0]) > 0 else ["No hay modelos para ese evento"]
        )
        models[1] = (
            models[1] if len(models[1]) > 0 else ["No hay modelos para ese evento"]
        )
        scalars = scalars if len(scalars) > 0 else ["No hay escalas para ese evento"]
        data_test = (
            data_test if len(data_test) > 0 else ["No hay datos para ese evento"]
        )
        self.selector_model_class["values"] = models[0]
        self.selector_model_linear["values"] = models[1]
        self.selector_scalar["values"] = scalars
