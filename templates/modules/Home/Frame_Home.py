...
class HomeFrame(ScrolledFrame):
    def __init__(self, master=None, data_emp=None, *args, **kwargs):
        ...
        # -----------------Title-------------------------------
        create_label(
            self, 0, 0, text="Inicio", font=("Helvetica", 30, "bold"), columnspan=2
        )
        # frame_wather = WeatherFrame(self)
        # frame_wather.grid(row=1, column=0, sticky="nsew", padx=(0, 10))
        self.frame_notifications = NotificationsFrame(self, **kwargs)
        self.frame_notifications.grid(row=2, column=0, sticky="we", padx=(0, 10))
        # -----------------Check permissions--------------------------------
        ...
        for index, dashboard in enumerate(self.dashboard_key):
            ...
            for window in avaliable_dashboards[dashboard]:
                ...
                frame.grid(
                    row=index_offset + index, column=0, sticky="we", padx=(0, 10)
                )
    ...
    def update_procedure(self, **events):
        for window in self.dashboard_frames:
            window.update_procedure(**events)
        self.frame_notifications.update_procedure(**events)
