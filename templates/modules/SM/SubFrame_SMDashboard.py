...
from static.constants import ventanasApp_path, status_dic
...
def create_plots(master, range_selected, data_chart=None):
    data_chart = (
        get_data_sm_per_range(range_selected, "normal")
...
class SMDashboard(ttk.Frame):
    def __init__(self, master=None, data=None, data_emp=None, *args, **kwargs):
        ...
        # ----------------------variables-------------------------------------
        self.svar_info_history = ttk.StringVar(
            value="************Selecciones un item de la tabla*********************"
        )
        self.svar_range_selector = ttk.StringVar(value="day")
        #  --------------------------graphs-----------------------------------
        self.frame_graphs = ttk.Frame(self)
        self.frame_graphs.grid(
            row=0, column=0, padx=(10, 25), pady=10, sticky="nswe", columnspan=1
        )
        ...
        self.range_selector = create_Combobox(
            master=frame_selector,
            row=0,
            column=0,
            values=["day", "month", "year"],
            width=10,
            textvariable=self.svar_range_selector,
            sticky="w",
        )
        ...
        self.plots = create_plots(
            self.frame_graphs,
            self.svar_range_selector.get(),
            data["data_dashboard"]["sm"]["data_chart"],
        )
        # --------------------------table SMs---------------------+--------------
        ...
        create_label(
            ...
            0,
            0,
            text="SMs en la base de datos",
            font=("Arial", 24, "bold"),
            columnspan=2,
        )
        ttk.Button(
            self.frame_table, text="Marca de recibido", command=self._on_recieved_sm
        ...
        if self.is_supper_user:
            ...
            self.table_events = self.create_table(
                self.frame_table,
                data=data["sm"]["data_sm_not_supper"],
                columns=data["sm"]["columns_sm"],
            )
        # ----------------------- history sm-----------------------------------
        self.info_history = ScrolledText(
            self.frame_table, padding=5, height=10, autohide=True
        )
    ...
    def create_table(self, master, data=None, columns=None):
        if data is None:
            self.data_sm, columns = get_all_sm_entries(
                filter_status=False, is_supper=self.is_supper_user, emp_id=self._id_emp
            )
        else:
            self.data_sm = data
    ...
    def on_double_click_table_sm(self, event):
        ...
        for i, item in enumerate(self.history):
            self.info_history.text.insert(
                ttk.END,
                f"Evento de {item['event']} por el usuario {item['user']} en la fecha: {item['date']}\n",
                tag_names[i % 2],
            )
        ...
        self.info_history.text.tag_config(
            "line1", foreground=self.style_gui.colors.get("warning")
        )
        self.info_history.text.tag_config(
            "line2", foreground=self.style_gui.colors.get("fg")
        )

    def on_reset_widgets_click(self):
        self.svar_info_history.set(
            "************Selecciones un item de la tabla*********************"
        )
    ...
    def _on_recieved_sm(self):
        ...
        if "Finalizado" in self.status_sm:
            Messagebox.show_error(
                title="Error", message="El material_request no esta en estado procesado"
            )
            return
