...
class BitacoraEditFrame(ScrolledFrame):
    def __init__(
        ...
        **kwargs,
        ...
        self.delta = delta
        self.emps_names = (
            kwargs["data"]["bitacora"]["emp_data"]
            if "bitacora" in kwargs["data"]
            else None
        )
        self.events = (
            kwargs["data"]["bitacora"]["events"]
            if "bitacora" in kwargs["data"]
            else None
        )
        self.columns = (
            kwargs["data"]["bitacora"]["columns"]
            if "bitacora" in kwargs["data"]
            else None
        )
        self.emp_id, self.data_emp = create_var_none(2)
        self.svar_info, self.svar_out, self.svar_activity, self.location = (
            create_stringvar(4, "")
        )
        self.bvar_prima = ttk.BooleanVar()
        # ----------------------------title------------------------------------
        create_label(
            self, 0, 0, text="Bitacora", font=("Helvetica", 30, "bold"), columnspan=2
        )
        ...
        (
            ...
            self.event_selector,
            self.label_valor,
            self.valor_entry_hora,
            self.valor_entry_mins,
            self.frame_valor,
            self.label_comment,
            self.comment_entry,
            self.prima_selector,
            self.emp_sel_contract,
            self.label_incidencia,
            self.incidencia_selector,
        ) = self.create_inputs(self.frame_inputs)
        create_label(
            self,
            2,
            0,
    ...
    def create_inputs(self, master):
        ...
        flag, error, emp_data = (
            ...
            if self.emps_names is None
            else (True, "", self.emps_names)
        )
        ...
        emp_name_contract = [
            i[1].upper() + " " + i[2].upper() for i in emp_data if i[3] == self.contrato
        ]
        ...
        emp_selector = ttk.Combobox(
            master, values=emp_name_extra, state="readonly", width=35
        )
        ...
        contract_selector = ttk.Combobox(
            master, values=contratos_display, state="readonly", width=15
        )
        contract_selector.grid(row=1, column=1, padx=10, pady=10, sticky="w")
        create_label(master, 2, 0, text="Fecha", sticky="w")
        date_selector = ttk.DateEntry(master, dateformat="%Y-%m-%d", firstweekday=0)
        ...
        event_selector = ttk.Combobox(
            master,
            values=["falta", "atraso", "extra", "normal"],
            state="readonly",
            width=15,
        )
    ...
    def create_buttons(self, master):
        ...
        btn_update_data = create_button(
            ...
            1,
            "Actualizar datos",
            command=self.on_update_data_event,
            sticky="n",
            width=15,
        )
        btn_reset = create_button(
            master, 0, 2, "Reset", command=self.on_reset_click, sticky="n", width=15
        )
    ...
    def prepare_data_to_send_DB(self):
        ...
        value = (
            float(self.valor_entry_hora.get())
            if self.valor_entry_hora.get() != ""
            else 0
        )
        value += (
            float(self.valor_entry_mins.get()) / 60
            if self.valor_entry_mins.get() != ""
            else 0
        )
    ...
    def on_add_click(self):
        ...
        (
            ...
            activity,
            location,
        ) = self.prepare_data_to_send_DB()
        out = check_date_difference(date, self.delta)
        if not out:
            self.svar_info.set(
                "No se puede añadir un evento pasado el tiempo limite de modificaciones."
            )
            return

        if include_prima and event == "extra":
            flag, error, result = update_bitacora(
                self.emp_id, event, (date, value, comment, contract)
            )
            if flag:
    ...
    def on_double_click_table(self, event):
        ...
        self.contract_selector.set(contract) if comment_dict[
            "contract"
        ] is None else self.contract_selector.set(comment_dict["contract"])
    ...
    def on_update_data_event(self):
        ...
        if flag:
            msg = (
                f"Record updated--> Empleado: {name}, "
    ...
    def create_table(self, master, hard_update=False):
        ...
        ttk.Label(
            master,
            text=f"Eventos del mes de {month_name}",
            font=("Helvetica", 22, "bold"),
        ).grid(row=0, column=0, columnspan=4, padx=10, pady=10)
        # tableview de eventos
        date = self.date_selector.entry.get()
        date = datetime.strptime(date, format_date)
        self.events, self.columns = (
            get_events_op_date(date, hard_update)
            if self.events is None or hard_update
            else (self.events, self.columns)
        )
        table_events = Tableview(
            master,
