...
from templates.Functions_GUI_Utils import create_label, create_Combobox
...
class StorageDashboard(ttk.Frame):
    def __init__(self, master, setting: dict = None, *args, **kwargs):
        ...
        self.create_plots(
            self.frame_plots,
            self._svar_selector.get(),
            10,
            kwargs["data"]["data_dashboard"]["almacen"]["data_chart"],
        )
    ...
    def create_plots(self, master, type_m, n_val, data_chart=None):
        ...
        self.plots = []
        data_chart = (
            get_data_movements_type(type_m, n_val) if data_chart is None else data_chart
        )
    ...
    def update_procedure(self, **events):
        self.create_plots(self.frame_plots, self._svar_selector.get(), 10)
