...
def new_employee(
    ...
    sql = (
        "INSERT INTO employees (name, l_name, curp, phone_number, email, department_id,"
...
def update_employee(
    ...
    sql = (
        "UPDATE employees SET name = %s, l_name = %s, curp = %s, phone_number = %s, email = %s, department_id = %s,"
...
def get_employee_id_name(name: str) -> tuple[None, str] | tuple[int, str]:
    ...
    """
    sql = (
        "SELECT employee_id, name, l_name FROM employees WHERE "
...
def get_id_name_employee(department: int, is_all=False):
    ...
    """
    sql = (
        "SELECT employee_id, name, l_name, puesto, date_admission, departure FROM employees "
...
def get_employees_op_names():
    sql = (
        "SELECT employee_id, name, l_name, contrato FROM employees "
...
def get_heads(limit=(0, 100)) -> list[list]:
    sql = (
        "SELECT heads.name, heads.employee, heads.department, departments.name, "
...
def get_chats(limit=(0, 100)):
    sql = (
        "SELECT chat_id, context, timestamp_start, timestamp_end, receiver_id, "
...
def get_tickets(limit=(0, 100)):
    sql = (
        "SELECT ticket_id, content_ticket, is_retrieved, is_answered, timestamp_create "
...
def get_users(limit=(0, 100)):
    sql = (
        "SELECT U_id, usernames, permissions,"
...
def insert_customer(
    ...
    sql = (
        "INSERT INTO sql_telintec.customers (name, l_name, phone_number, city, email) "
        "VALUES (%s, %s, %s, %s, %s)"
    )
...
def update_customer_DB(
    ...
    sql = (
        "UPDATE sql_telintec.customers SET name = %s, l_name = %s, phone_number = %s, "
...
def update_department_DB(
    name: str, location: str, department_id: int
) -> tuple[bool, Exception | None, int | None]:
    sql = (
        "UPDATE sql_telintec.departments SET name = %s, location = %s "
        "WHERE department_id = %s"
    )
...
def insert_head(
    position_name: str, department: str, employee: str
) -> tuple[bool, Exception | None, int | None]:
    sql = (
        "INSERT INTO sql_telintec.heads (name, department, employee) "
        "VALUES (%s, %s, %s)"
    )
...
def update_head_DB(
    position_name: str, department: str, employee: str, head_id: int
) -> tuple[bool, Exception | None, int | None]:
    sql = (
        "UPDATE sql_telintec.heads SET name = %s, department = %s, employee = %s "
        "WHERE position_id = %s"
    )
...
def update_supplier_DB(
    ...
    sql = (
        "UPDATE sql_telintec.suppliers SET name = %s, location = %s "
        "WHERE supplier_id = %s"
    )
...
def update_product_and_service(
    ...
    sql = (
        "UPDATE sql_telintec.products_services SET name = %s, model = %s, marca = %s, "
...
def insert_order(
    ...
    sql = (
        "INSERT INTO sql_telintec.orders (order_id, product_id, quantity, date_order, "
...
def update_order_db(
    ...
    sql = (
        "UPDATE sql_telintec.orders SET product_id = %s, quantity = %s, date_order = %s, "
...
def insert_vorder_db(
    ...
    sql = (
        "INSERT INTO sql_telintec.virtual_orders (vo_id, products, date_order, "
...
def update_vorder_db(
    ...
    sql = (
        "UPDATE sql_telintec.virtual_orders SET products = %s, date_order = %s, "
...
def insert_ticket_db(
    id_t: int, content: str, is_retrieved: int, is_answered: int, timestamp: str
):
    sql = (
        "INSERT INTO sql_telintec.tickets (ticket_id, content_ticket, is_retrieved, is_answered, "
...
def update_ticket_db(
    ...
    sql = (
        "UPDATE sql_telintec.tickets SET content_ticket = %s, is_retrieved = %s, is_answered = %s, "
...
def get_username_data(username: str):
    sql = (
        "select users_system.exp, users_system.timestamp_token, employees.name,"
...
def get_all_data_employees(status: str):
    if "all" in status.lower():
        ...
        status = "ACTIVO"
    sql = (
        "SELECT employees.employee_id, employees.employee_id, employees.name, employees.l_name, "
...
def get_all_fichajes():
    ...
    sql = (
        "SELECT employees.name, employees.l_name, "
...
def get_fichaje_DB(emp_id: int):
    sql = (
        "SELECT ficha_id, emp_id, contract, absences, lates, extras, primes, normal "
        "FROM sql_telintec.fichajes "
        "WHERE emp_id = %s"
    )
    val = (emp_id,)
...
def update_aptitud_renovacion(
    aptitud: list, renovaciones: list, apt_actual: int, last_date: str, emp_id: int
):
    sql = (
...
def verify_user_DB(user: str, password: str) -> bool:
    ...
    :return: <Boolean>
    """
    sql = "SELECT usernames FROM users_system " "WHERE usernames = %s AND password = %s"
    val = (user, password)
    flag, error, result = execute_sql(sql, val)
...
def get_all_employees_active():
    sql = (
        "SELECT employee_id, name, l_name, date_admission "
...
def get_products_almacen(id_p: int, name: str, category: str, limit: int = 10):
    ...
    sql = (
        "SELECT id_product, name, udm, stock, id_category FROM products_amc WHERE id_product = %s OR "
...
def get_high_stock_products(category: str, quantity: int):
    columns = ("id_product", "name", "udm", "stock", "id_category")
    # get category
    sql = "SELECT id_category, name " "FROM product_categories_amc " "WHERE name = %s"
    val = (category.lower(),)
    flag, error, result = execute_sql(sql, val, 1)
    if len(result) > 0:
...
def get_orders_amc(id_o: int, id_c: int, status: str, name_c: str, date: str):
    ...
    if id_c is None:
        ...
        if len(result) > 0:
            ...
            id_c = "%"
    sql = (
        "SELECT id_order, id_customer, status, sm_code, order_date, id_customer "
...
def get_employee_info(id_e: int):
    ...
    sql = (
        "SELECT employee_id, name, l_name, date_admission, status, department_id, phone_number, email, contrato "
        "FROM employees "
        "WHERE employee_id = %s"
    )
    val = (id_e,)
...
def get_sm_employees():
    sql = "SELECT employee_id, name, l_name " "FROM employees WHERE status = 'activo' "
    flag, error, result = execute_sql(sql, None, 5)
    return flag, error, result


def get_sm_clients():
    sql = "SELECT id_customer, name " "FROM customers_amc  "
    flag, error, result = execute_sql(sql, None, 5)
    return flag, error, result


def get_sm_products():
    sql = "SELECT id_product, name, udm, stock " "FROM products_amc "
    flag, error, result = execute_sql(sql, None, 5)
    return flag, error, result
...
def insert_sm_db(data):
    ...
    sql = (
        "INSERT INTO materials_request (sm_code, folio, contract, facility, location, "
...
def delete_sm_db(id_m: int, sm_code: str):
    ...
    sql = (
        "SELECT sm_id, sm_code, folio, contract, facility, location, client_id, emp_id, date, limit_date, "
...
def update_sm_db(data):
    ...
    sql = (
        "UPDATE materials_request SET sm_code = %s, folio = %s, contract = %s, facility = %s, location = %s, "
...
def cancel_sm_db(id_m: int, history: dict):
    ...
    if id_m not in ids_sm:
        return True, "Material request not found", None
    sql = "UPDATE materials_request SET status = -1, history = %s  " "WHERE sm_id = %s "
...
def update_history_sm(sm_id, history: dict, items: list, is_complete=False):
    if is_complete:
        sql = (
            "UPDATE materials_request SET history = %s, status = 2 " "WHERE sm_id = %s "
        )
        val = (json.dumps(history), sm_id)
        flag, error, result = execute_sql(sql, val, 4)
    else:
        sql = (
            "UPDATE materials_request SET history = %s, status = 1  "
            "WHERE sm_id = %s "
        )
        val = (json.dumps(history), sm_id)
        flag, error, result = execute_sql(sql, val, 4)
