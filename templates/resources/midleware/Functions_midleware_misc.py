...
def get_response_AV(department: str, msg: str, files_av: list, filename: str, id_chat=None):
    ...
    files_av, res = get_response_assistant(msg, filename, files_av, instructions, department)
...
def get_events_from_extraordinary_sources(hour_in: str, hour_out: str, data: dict):
    ...
    if normal_hour_in >= timestamp_hour_in >= timestamp_limit_hour_in and normal_hour_out >= timestamp_hour_out >= timestamp_limit_hour_out:
        print(f"event: normal, date: {data['date']}, value: {1.0}, comment: {data['comment']}")
