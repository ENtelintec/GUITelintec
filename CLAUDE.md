...
Per-change design notes live in [`docs/`](docs/) (Spanish, one `.md` per change, relative `../` links into the code). **Index: [`docs/README.md`](docs/README.md)** — one line per doc, grouped by business area, chronological inside each section. Read only the section of the area you are touching, then open the docs it points to; don't load the whole folder.

When you make a non-trivial change — new/altered endpoint, cross-layer refactor, schema/contract change — add a short doc in `docs/` following the existing style ([`heads_crud_fields_sync.md`](docs/heads_crud_fields_sync.md) is a good template: the 4 layers touched, what changed, and an "Al modificar" section). Every new doc carries three obligations:

1. **Always include a front contract block** in any doc that adds/changes an endpoint: a "Contrato mínimo para el front" section with the concrete request/response contract so a front agent can integrate without reading the backend. It must state (at minimum) the **auth header** — `Authorization` carries the **raw JWT, NOT `Bearer <token>`** (`jwt.decode` on the raw header, no prefix stripping) — the base path, request shape (fields/types, multipart is one-file-per-request), response content-types (the `{data,msg,error}` JSON envelope; **file downloads return a binary blob on 200 and the JSON envelope on 4xx** — the front branches on status), concrete success/error examples per status code, and any gotchas (truthy query values, etc.). [`remission_combined_pdf.md`](docs/remission_combined_pdf.md) has a reference block.
2. **Add one line to [`docs/README.md`](docs/README.md)**, at the **end** of its area section (sections are chronological ascending), copying the format of the existing lines: link to the doc — main endpoint in backticks (if it has one): what it does, ≤ ~25 words, no bug history · `YYYY-MM-DD` · `⏳` + the open item in a few words. The `⏳` part only while the doc has an open item (it is a pointer — the source of truth is `pendientes.md`; remove it when the item closes). One entry per doc: a cross-area doc lives where its main endpoint lives and gets a link in the other section's `Ver también` line. Later facts about an existing doc go **in that doc** (a `## Notas posteriores` section at the end), never in the index.
3. **Reflect open/closed items in [`docs/pendientes.md`](docs/pendientes.md)** — los pendientes abiertos de **todas** las áreas (back y front) viven consolidados ahí, un listado con link al doc completo de cada ítem; al cerrar o crear un pendiente en cualquier doc, reflejarlo ahí.

Doc lifecycle: a grill/proposal that will end in code is born in the `docs/` root with an `Estado: propuesta` line and **never moves** (when implemented only the `Estado` changes — moving a doc breaks its `../` links and every link pointing at it). Calendar / multi-phase plans live in [`docs/planes/`](docs/planes/) (`plan_mes_N.md`, `<iniciativa>_plan.md`); closed plans and discarded grills go to [`docs/planes/archivo/`](docs/planes/archivo/). The folder is `docs/` lowercase (that is what git tracks — `Docs/` links break outside Windows).
